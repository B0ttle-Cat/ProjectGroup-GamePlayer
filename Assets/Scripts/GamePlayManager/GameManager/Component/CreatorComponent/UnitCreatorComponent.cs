...
{
	public class UnitCreatorComponent : ComponentBehaviour, IUnitCreatorComponent
	{
		...
		public override void BaseAwake()
		{
			...
			gamePlayManager = ThisContainer.GetParentObject<IGamePlayManager>();
		...
		private void CreateObject(CreateSettingInfo createSetting)
		{
			...
			if(!ThisContainer.TryGetChildObject<FireunitObject>(out var _object, item => FindObject(item, key)))
			{
				...
				if(!unitObject.ThisContainer.TryGetComponent<UnitInitializeSetter>(out var unitInitSetter))
				{
					unitInitSetter = unitObject.ThisContainer.AddComponent<UnitInitializeSetter>();
				}
				unitInitSetter.fireunitData = data;
				//TODO
				unitInitSetter.gamePlayManager = gamePlayManager;
